...
@section('content')
    ...
    <div class="container mt-3">
        <form action="{{ route('kasir.order.store')}}" class="card m-auto p-5" method="POST">
            ...
            @endif
            <p>Penaggung Jawab : <b> {{ Auth::user()->name}} </b></p>
            <div class="mb-3 row">
                <label for="name_customer" class="col-sm-2 col-from-label">Nama Pembeli</label>
                <div class="col-sm-10">
            ...
            <div class="mb-3 row">
                ...
                <div class="col-sm-10">
                    {{-- nama dibuat array karena nantinya data obat (medicines) akan berbentuk array/data bisa lebih dari satu --}}
    ...
    <script>
        ...
        $("#add-select").on("click", function() {
            ...
            let el=`<br><select name="medicines[]" id="medicines" class="form-select">
                    <option selected hidden disable>pesan ${no}</option>
                    @foreach ($medicines as $item)
                <option value="{{ $item['id'] }}">{{ $item['name']}}</option>
            @endforeach
            </select>`;
        // append : tambahkan element html dibagian sblm penutup tag penutup terkait (sblm penutup tag yang id nya wrap-medicines)
        $("#wrap-medicines").append(el);
        // increments variable no agar  angka yang muncul di option selalu bertambah 1 jumlah selectnya
        no++;
        });
    </script>
@endpush