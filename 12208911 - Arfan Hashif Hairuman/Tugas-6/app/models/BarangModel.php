...
class BarangModel {
    ...
    public function tambahBarang($data) {
        ...
        $this->db->query($query);
        $this->bindDataAndExecute($data);

        return $this->db->rowCount();
    }

    public function updateDataBarang($data) {
        if ($this->isValidTglKembali($data)) {
    ...
    private function bindDataAndExecute($data) {
    ...
    private function isValidTglKembali($data) {
        return ($data['tgl_kembali'] != '0000-00-00 00:00:00' && $data['tgl_kembali'] > $data['tgl_pinjam']);
    }
}
